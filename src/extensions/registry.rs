...
use crate::parser::ScopedParse;
use crate::textify::{ErrorQueue, OutputOptions, Scope, ScopedContext};
...
impl<'a> ExplainContext<'a> {
    ...
    /// Convert a [`proto::Type`] to its text-format representation.
    pub fn type_to_string(&self, ty: &proto::Type) -> Result<String, ExtensionError> {
        let options = OutputOptions::default();
        let errors = ErrorQueue::default();
        let registry = ExtensionRegistry::new();
        let scope = ScopedContext::new(&options, &errors, self.extensions, &registry);
        let rendered = format!("{}", scope.display(ty));
        errors.errs().map_err(|errors| {
            ExtensionError::InvalidArgument(format!("Error formatting type: {errors}"))
        })?;
        Ok(rendered)
    }

    /// Parse a text-format type string into a [`proto::Type`].
    pub fn string_to_type(&self, s: &str) -> Result<proto::Type, ExtensionError> {
        proto::Type::parse(self.extensions, s)
            .map_err(|e| ExtensionError::InvalidArgument(format!("Error parsing type '{s}': {e}")))
    }
