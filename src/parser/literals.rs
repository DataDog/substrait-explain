...
pub(crate) struct ParsedLiteral {
    ...
    pub(crate) explicit_type: Option<Type>,
}
...
impl ScopedParsePair for Literal {
    ...
    fn parse_pair(
        ...
        parse_literal_pair(extensions, pair).map(|parsed| parsed.literal)
    }
}
...
pub(crate) fn parse_literal_pair(
    extensions: &SimpleExtensions,
    pair: pest::iterators::Pair<Rule>,
) -> Result<ParsedLiteral, MessageParseError> {
    match pair.as_rule() {
        Rule::literal => {
            let mut pairs = pair.into_inner();
            let value = unwrap_single_pair(pairs.next().unwrap());
            let typ = pairs.next();
            assert!(pairs.next().is_none());

            let syntax = LiteralSyntax::from_pair(value);
            let explicit_type = match typ {
                Some(t) => Some(Type::parse_pair(extensions, t)?),
                None => None,
            };
            let target = LiteralTarget::from_type(explicit_type.clone(), &syntax)?;
            let literal = target.parse(syntax)?;
            Ok(ParsedLiteral {
                literal,
                explicit_type,
            })
