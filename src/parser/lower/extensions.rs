...
pub(crate) fn build_extension_args_from_relation(
    ...
    for arg in &relation.args.positional {
        args.positional.push(extension_value_from_arg(arg).map_err(|e| plan_error(ctx, e))?);
    }
    ...
    for arg in &relation.outputs {
        args.output_columns.push(extension_column_from_arg(arg).map_err(|e| plan_error(ctx, e))?);
    }
