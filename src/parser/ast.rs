...
pub enum Expr {
    ...
    IfThen(IfThenExpr),
    Identifier(String),
    /// A dot-separated name like `schema.table` or a single quoted name like
    /// `"sales.q1"`. Each element is one segment.
    DottedName(Vec<String>),
}
...
impl Expr {
    ...
    pub fn dotted_name(names: Vec<String>) -> Self {
        if names.len() == 1 {
            Self::Identifier(names.into_iter().next().unwrap())
        } else {
            Self::DottedName(names)
        }
    }
}
