...
use crate::parser::errors::{MessageParseError, ParseContext, ParseError, ParseResult};
...
impl LineNode {
    ...
    pub fn parse(line: &str, line_no: i64) -> Result<Self, ParseError> {
        let relation = lalrpop_line::parse_relation(line).map_err(|e| {
            ParseError::Plan(
                ...
                MessageParseError::invalid("relation", e),
            )
        })?;
    ...
    pub fn parse_root(line: &str, line_no: i64) -> Result<Self, ParseError> {
        ...
        let relation = lalrpop_line::parse_relation(line).map_err(|e| {
            ParseError::Plan(
                ParseContext::new(line_no, line.to_string()),
                MessageParseError::syntax("top_level_relation", e),
            )
        })?;
