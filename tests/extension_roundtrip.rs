...
fn extension_table_test_registry() -> ExtensionRegistry {
    let mut registry = ExtensionRegistry::new();
    registry
        .register_extension_table::<UserTableConfig>()
        .unwrap();
    registry
}

fn assert_extension_table_parse_error_contains(plan_text: &str, expected: &str) {
    let parser = Parser::new().with_extension_registry(extension_table_test_registry());
...
#[test]
fn test_extension_table_read_missing_ext_addendum_fails() {
    let plan_text = r#"
=== Plan
Root[id]
...
"#;
    ...
    assert_extension_table_parse_error_contains(
        plan_text,
...
#[test]
fn test_extension_table_read_duplicate_ext_addendum_fails() {
    let plan_text = r#"
=== Plan
Root[id]
  Read:Extension[id:i64]
...
#[test]
fn test_ext_addendum_on_named_read_fails() {
    let plan_text = r#"
=== Plan
Root[id]
  Read[data => id:i64]
...
#[test]
fn test_ext_addendum_on_extension_relation_fails() {
    let mut registry = extension_table_test_registry();
    registry.register_relation::<UserTableConfig>().unwrap();
...
"#;
    ...
    let result = parser.parse_plan(plan_text);
    match result {
        Ok(_) => panic!("expected parse error for addendum on extension relation"),
        Err(error) => {
            let message = error.to_string();
            assert!(
                message.contains("extension relations do not support addenda"),
                "expected extension-relation addendum error, got: {message}"
            );
        }
    }
}
