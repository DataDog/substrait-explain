...
A literal can come in the form of an integer, float, boolean, string, or typed null. Non-null literals can have an optional additional type:

`literal := ((float / integer / boolean / string) (":" type)?) / ("null" ":" type)`
...
- **`null`**` := "null" ":" type`
  ...
  - Null literals must include a type annotation
